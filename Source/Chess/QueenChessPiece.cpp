...
{
    return Super::GetValue() * 900 + (bIsWhite ? QueenPieceSquareTable[CurrentPosition.X][CurrentPosition.Y] : -QueenPieceSquareTable[UKismetMathLibrary::Abs_Int(7 -CurrentPosition.X)][UKismetMathLibrary::Abs_Int(7 -CurrentPosition.Y)]);
}

TArray<FIntPoint> AQueenChessPiece::GetPossibleMovePositionsForEnemy()
{
    TArray<FIntPoint> PossibleMoves;
    if(ChessBoard)
    {
        ...
        for(int i{2}; i <= AChessBoard::BoardHeight; i++)
        {
            for(int j{}; j < diag; j++)
            {
                if(IsIndexValid(Diagonal[j]) && !PossibleMoves.Contains(Diagonal[j]))
                {
                    PossibleMoves.Emplace(Diagonal[j]);
                }
                else 
                ...
                if(ChessBoard->GetChessPiece(Diagonal[j]) && (ChessBoard->GetChessPiece(Diagonal[j])->GetType() != Type::King || ChessBoard->GetChessPiece(Diagonal[j])->IsWhite() == bIsWhite))
                {
                    continue;
                }
                switch(j)
            ...
            for(int j{}; j < direc; j++)
            {
                if(IsIndexValid(Direction[j]) && !PossibleMoves.Contains(Direction[j]))
                {
                    PossibleMoves.Emplace(Direction[j]);
                }
                else 
                ...
                if(ChessBoard->GetChessPiece(Direction[j]) && (ChessBoard->GetChessPiece(Direction[j])->GetType() != Type::King || ChessBoard->GetChessPiece(Direction[j])->IsWhite() == bIsWhite))
                {
                    continue;
                }
                switch(j)
